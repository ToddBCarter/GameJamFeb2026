...
public class FinishLine : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Racer")) {
            ...
            if (racer.returnCheckpointValue()) racer.incLaps();

            int CurrentLap = racer.getLaps();

            Debug.Log(other.name + ": Lap " + CurrentLap + "/" + TotalLaps);

            if ((CurrentLap >= TotalLaps) && (racer.returnCheckpointValue())) {
                ...
                racer.checkpointFalse(); //regardless of if the player cheated or completed a lap, this will just reset the flag
            }
        }
    }
}
